...
def start() -> None:
    ...
    attachment = asyncio.run(_put_image('dialx-banner.png'))
    second_attachment = asyncio.run(_put_image('Elephant-male.jpg'))
    ...
    dial_client.get_completion(messages=[
        Message(
            role=Role.USER,
            content="What do you see on this picture?",
            custom_content=CustomContent(attachments=[attachment,second_attachment])
        )
    ])
